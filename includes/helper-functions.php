	...
	if( array_key_exists( 'events', $page_settings ) ) {
		...
		if( ! $post_id ) {
			$pages['events'] = wp_insert_post( array(
				...
				'comment_status' => 'closed',
				'ping_status'    => 'closed',
				'post_content'   => '[aec_map]<br />[aec_events]',
				'post_status'    => 'publish',
				'post_author'    => get_current_user_id()
			));	
		}
	...
	if( array_key_exists( 'category', $page_settings ) ) {
		...
		if( ! $post_id ) {
			$pages['category'] = wp_insert_post( array(
				...
				'comment_status' => 'closed',
				'ping_status'    => 'closed',
				'post_content'   => '[aec_map]<br />[aec_category]',
				'post_status'    => 'publish',
				'post_author'    => get_current_user_id()
			));
		}
	...
	if( array_key_exists( 'tag', $page_settings ) ) {
		...
		if( ! $post_id ) {
			$pages['tag'] = wp_insert_post( array(
				...
				'comment_status' => 'closed',
				'ping_status'    => 'closed',
				'post_content'   => '[aec_map]<br />[aec_tag]',
				'post_status'    => 'publish',
				'post_author'    => get_current_user_id()
			));		
		}
	...
	if( array_key_exists( 'organizer', $page_settings ) ) {
		...
		if( ! $post_id ) {
			$pages['organizer'] = wp_insert_post( array(
				...
				'comment_status' => 'closed',
				'ping_status'    => 'closed',
				'post_content'   => '[aec_map]<br />[aec_organizer]',
				'post_status'    => 'publish',
				'post_author'    => get_current_user_id()
			));
		}
...
function the_aec_address( $venue_id ) {
	...
	if( $website = get_post_meta( $venue_id, 'website', true ) ) {
		$meta[] = sprintf( '<a href="%s" target="_blank" style="word-break: break-all;">%s</a>', $website, $website );
	}
 ...
 * @since    1.7.0
 */
function the_footer_text() {
	
	$general_settings = get_option( 'aec_general_settings' );
	
	if( ! empty( $general_settings['show_credit_link'] ) ) {
		echo '<p style="font-size: 12px; margin-top: 10px;">Powered by <a href="https://yendif.com/wordpress/item/another-events-calendar.html" target="_blank">Yendif Technologies</a></p>';
	} 
...
function aec_get_event_date( $post_id ) {
	...
	if( $all_day_event ) {
		...
		$formatted_date = '<span class="aec-date">' . date_i18n( get_option( 'date_format' ), strtotime( $start_date_time ) ) . '</span>';
		...
		if( '0000-00-00 00:00:00' != $end_date_time ) {
			...
			if( $day_diff > 0 ) {
				$formatted_date .= '<span class="aec-hyphen-separator">-</span>';
				$formatted_date .= '<span class="aec-date">' . date_i18n( get_option( 'date_format' ), strtotime( $end_date_time ) ) . '</span>';
			}
		}
		
	} else {
		
		$start_date = '<span class="aec-date">' . date_i18n( get_option( 'date_format' ), strtotime( $start_date_time ) ) . '</span>';
		$start_time = '<span class="aec-time">' . date_i18n( get_option( 'time_format' ), strtotime( $start_date_time ) ) . '</span>';
		$formatted_date = $start_date . '<span class="aec-space-separator">&nbsp;</span>' . $start_time;
		...
		if( '0000-00-00 00:00:00' != $end_date_time ) {
			...
			if( $day_diff < 1 ) {
				$formatted_date .= '<span class="aec-hyphen-separator">-</span>';
				$formatted_date .= '<span class="aec-time">' . date_i18n( get_option( 'time_format' ), strtotime( $end_date_time ) ) . '</span>';
			}
			...
			// Else
				...
				$end_date = '<span class="aec-date">' . date_i18n( get_option( 'date_format' ) , strtotime( $end_date_time ) ) . '</span>';
				$end_time = '<span class="aec-time">' . date_i18n( get_option( 'time_format' ), strtotime( $end_date_time ) ) . '</span>';
				$formatted_date .= $end_date . '<span class="aec-space-separator">&nbsp;</span>' . $end_time;	
			}
		}
