...
class  AEC_Public_User {
	 ...
	 * @param    int       $occurences       Number of Occurences.
 	 */
	public function process_monthly_events( $main_event_id, $meta, $start_date = '', $occurences = 0 ) {
	...
	public function shortcode_aec_manage_venues() {     
		...
		if( empty( $general_settings['has_venues'] ) ) {
			return __( 'Event Venues disabled by the admin.', 'another-events-calendar' );
		}
	...
	public function shortcode_aec_manage_organizers() {     
		...
		if( empty( $general_settings['has_organizers'] ) ) {
			return __( 'Event Organizers disabled by the admin.', 'another-events-calendar' );
		}
