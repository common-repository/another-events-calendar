        		...
        		<div class="row aec-table-layout-row">
            		...
            		<div class="col-md-6">                	
                        ...
                        <?php endif; ?>
                         ...
                         <?php 
						 	if( ! empty( $general_settings['has_categories'] ) ) {
								$categories = get_the_terms( $event->ID, 'aec_categories' );
								if( ! empty( $categories ) ) {
									$meta = array();
									foreach( $categories as $category ) {
										$meta[] = sprintf( '<a class="text-primary aec-margin-left" href="%s">%s</a>', aec_category_page_link( $category ), $category->name );
									}
									echo '<p class="aec-margin-top aec-no-margin-bottom text-muted"><span class="glyphicon glyphicon-folder-open"></span> '.implode( ', ', $meta ).'</p>';
								}
							}
						?>
                        ...
                        <?php 
							if( ! empty( $general_settings['has_venues'] ) ) {
								$venue_id = get_post_meta( $event->ID, 'venue_id', true );
