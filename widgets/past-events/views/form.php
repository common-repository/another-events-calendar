...
<?php if( ! empty( $general_settings['has_categories'] ) ) : ?>
    <p>
        <input class="checkbox" type="checkbox" <?php checked( $show_category, 1 ); ?> id="<?php echo $this->get_field_id( 'show_category' ); ?>" name="<?php echo $this->get_field_name( 'show_category' ); ?>" value="1" /> 
        <label for="<?php echo $this->get_field_id( 'show_category' ); ?>"><?php _e( 'Show Category', 'another-events-calendar' ); ?></label>
    </p>
<?php endif; ?>
