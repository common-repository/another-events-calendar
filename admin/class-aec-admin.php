...
class AEC_Admin {
	...
	public function manage_upgrades() {    
		...
		if( AEC_PLUGIN_VERSION !== get_option( 'aec_version' ) ) {
		
			$default_pages_values = aec_insert_pages();
			update_option( 'aec_page_settings', $default_pages_values );
			
			$general_settings = get_option( 'aec_general_settings' );
			if( ! array_key_exists( 'has_categories', $general_settings ) ) {
				$general_settings['has_categories'] = 1;
				update_option( 'aec_general_settings', $general_settings );
			}
